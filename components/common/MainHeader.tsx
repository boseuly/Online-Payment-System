...
export default function MainHeader({
    ...
    useEffect(() => {
        ...
        // 같은 탭에서 변경 즉시 반영 될 수 있도록 커스텀 이벤트 생성
        window.addEventListener("accessTokenChanged", sync as EventListener);
        window.dispatchEvent(new Event("accessTokenChanged"));

        return () => {
            window.removeEventListener("accessTokenChanged", sync as EventListener);
            // 탭b 또한 자동으로 변경된 걸 감지할 수 있도록 storage 이벤트 발생하고, sync() 실행
    ...
    // 로그아웃 실행
    const logout = () => {
        const res = apiFetch("api/v1/customer/logout", {method: "POST"});
        localStorage.setItem("accessToken", "");
        localStorage.setItem("refreshToken", "");
        const sync = () => setAccessToken("");
        window.addEventListener("accessTokenChanged", sync as EventListener);
        window.dispatchEvent(new Event("accessTokenChanged"));
    }
                ...
                <div className="d-flex m-15 flex-justify-space-between ">
                    ...
                    <div className="d-flex gap-4">
                        {!accessToken ?
                            <>
                                <div>
                                    <p className={`${color == "black" ? "text-black" : "text-white"} text-15 default_font cursor-point`}
                                       onClick={() => {
                        ...
                        <div>
                            <FiSearch size={"20"} color={`${color == "black" ? "black" : "white"}`}
                                      className="cursor-point"/>
                        </div>
                        <div>
                            <FiShoppingCart size={"20"} color={`${color == "black" ? "black" : "white"}`}
                                            className="cursor-point"/>
                        </div>
                        {!hamburgerOpenYn ? (
                            <div onClick={setHamburgerOpenYn}>
                                <FiMenu size={"20"} color={`${color == "black" ? "black" : "white"}`}
                                        className="cursor-point"/>
                            </div>
